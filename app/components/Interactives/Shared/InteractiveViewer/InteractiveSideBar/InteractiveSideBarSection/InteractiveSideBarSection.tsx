...
import { InteractiveSideBarNode } from "../InteractiveSideBarPage/InteractiveSideBarPage";
...
export type InteractiveSideBarSectionNode = {
  title: string;
  active: boolean;
  locked?: boolean;
...
export default function InteractiveSideBarSection({
  node,
  first,
  inner,
  currentView,
  ...
  setCurrentView: any;
  first?: boolean;
  node: InteractiveSideBarSectionNode;
  inner?: boolean;
              ...
              return (
                ...
                >
                  <InteractiveSideBarSection
                    additionalPadding={
                      additionalPadding ? additionalPadding + 200 : 0
                    }
                    ...
                    node={{
                      ...
                      subcomponents: subNode.subcomponents.map(
                        (subComponent, idx) => {
                          ...
                          return {
                            active:
                              ...
                              currentView.section === subNode.sectionId,
                            complete:
                              typeof subComponent?.completionKey === "number" &&
                              subNode.completionAtom
                                ? useAtomValue(subNode.completionAtom)[
            ...
            return (
              <button
                onClick={() => {
                  ...
                  } else if ("callback" in subNode && subNode.callback) {
              ...
              >
                <h4 className="col-start-2 text-sm">{subNode.title}</h4>
                {!node.locked &&
                (subNode.requires === null || subNode.requires) ? (
                  <div
                    className={`absolute right-4 top-1/2 h-2 w-2 -translate-y-1/2 rounded-full border-[1px] border-zinc-600
                    ${subNode?.complete ? "bg-current" : ""}`}
                  ></div>
                ) : null}
              </button>
            );
