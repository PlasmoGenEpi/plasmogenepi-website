...
export default function StepOneNode() {
  ...
  const title = "Step 1";
  const sectionActive = currentView.section === 1;
  const sectionLocked = !completion?.[0]?.[2];
        ...
        >
          ...
          >
            <h4 className="col-start-2 text-sm pr-8">
              Match sequencer generated reads to reference genome
            </h4>
            <div
              className={`absolute right-4 top-1/2 h-2 w-2 -translate-y-1/2 rounded-full border border-zinc-600
                     ${completion?.[1]?.[0] ? "bg-current" : ""}`}
            ></div>
          </button>
          <button
            disabled={sectionLocked || !completion?.[1]?.[0]}
            onClick={() => {
              if (isRecentlyClicked) {
                setSideBarIsOpen(false);
              } else {
