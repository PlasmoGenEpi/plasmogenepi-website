...
export default function StepTwoNode() {
  ...
  const title = "Step 2";
  const sectionActive = currentView.section === 2;
  const sectionLocked = !completion?.[1]?.[1];
      ...
      >
        ...
        >
          <button
            ...
            // }`}
          >
            <h4 className="col-start-2 text-sm pr-8">
              Match sequencer generated reads to reference genome &ndash; this
            ...
            <div
              className={`absolute right-4 top-1/2 h-2 w-2 -translate-y-1/2 rounded-full border border-zinc-600
                     ${completion?.[2]?.[0] ? "bg-current" : ""}`}
            ></div>
          </button>
          <button
            disabled={sectionLocked || !completion?.[2]?.[0]}
            onClick={() => {
              if (isRecentlyClicked) {
                setSideBarIsOpen(false);
              } else {
          ...
          >
            <h4 className="col-start-2 text-sm pr-8">Knowledge Check</h4>
            <div
              className={`absolute right-4 top-1/2 h-2 w-2 -translate-y-1/2 rounded-full border border-zinc-600
                     ${completion?.[2]?.[1] ? "bg-current" : ""}`}
            ></div>
          </button>
          <button
            disabled={sectionLocked || !completion?.[2]?.[1]}
            onClick={() => {
              if (isRecentlyClicked) {
                setSideBarIsOpen(false);
              } else {
