...
export default function SideBar4SubComponent({
  ...
  useEffect(() => {
    if (
      section.subcomponents
        .map((subComp) => {
          return subComp.isActive(currentView);
        })
        .filter((bool) => {
          return bool;
        }).length &&
  ...
  return (
    <button
      ...
      className={`block w-full border-b border-white/20 md:w-[384px] 
        ${
        ...
        <div className="col-start-3">
          {section.sectionComplete ? (
            <svg
              ...
              height="20pt"
              version="1.1"
              className={` stroke-interactiveBlue/  fill-current stroke-[2.25rem] text-current`}
              viewBox="0 0 1200 1200"
              xmlns="http://www.w3.org/2000/svg"
            >
